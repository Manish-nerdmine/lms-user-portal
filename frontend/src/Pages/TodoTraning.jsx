...
const TodoTraining = ({ groupId, overdueCourses, setOverdueCourses, completedCourses, setCompletedCourses }) => { 
  ...
  useEffect(() => { 
    const fetchGroupCourses = async () => { 
      try { 
        ...
        console.log("Fetching courses for GroupId:", storedGroupId, "and uId:", uId); 
        ...
        allCourses.forEach((c) => { 
          ...
          const progress = Number(localStorage.getItem(`progress_${userId}_${c.courseId}`)) || 0; 
          const savedCompletion = localStorage.getItem(`completedDate_${userId}_${c.courseId}`); 
          ...
          if (progress === 100) { 
            ...
            localStorage.setItem(`completedDate_${userId}_${c.courseId}`, completionDate); 
          ...
          {courses.length === 0 ? ( 
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-10"> 
              {courses.map((c, index) => { 
                ...
                const progress = localStorage.getItem(`progress_${userId}_${c.courseId}`) || 0; 

                return ( 
                  <div key={c.courseId || index} className="bg-white p-4 rounded-lg shadow hover:shadow-lg transition"> 
                    {/* Thumbnail */} 
                    {details.thumbnail && details.thumbnail !== "null" ? ( 
                      <img src={details.thumbnail} alt={details.title} className="w-full h-40 object-cover rounded mb-3" /> 
                    ) : ( 
                    ...
                    <div className="flex justify-between items-center mb-1"> 
                      ...
                      <span className={`text-xs px-2 py-1 rounded ${ details.isActive ? "bg-green-100 text-green-600" : "bg-red-100 text-red-600" }`}> 
                    ...
                    <div className="w-full bg-gray-200 rounded-full h-2 mb-2"> 
                      <div className="bg-purple-600 h-2 rounded-full transition-all duration-500" style={{ width: `${ progress ? `${progress}%` : "0%" }` }}></div> 
                    </div> 
